...
show_post_install_steps() {
    ...
    if [ "${POST_INSTALL_LOGIN_STATUS:-skipped}" != "success" ]; then
...
# Verify installation
verify_installation() {
    if CR_CLI_DOCTOR_SOURCE=install_sh PATH="$BIN_DIR:$PATH" coderabbit doctor >/dev/null 2>&1; then
        print_success "Installation verified"
    else
        print_warning "CLI install could not be verified with 'coderabbit doctor'"
    fi
}

run_post_install_login() {
    POST_INSTALL_LOGIN_STATUS="skipped"
    POST_INSTALL_LOGIN_INTERRUPTED="0"
    ...
    post_install_login_interrupted() {
        POST_INSTALL_LOGIN_STATUS="interrupted"
        POST_INSTALL_LOGIN_INTERRUPTED="1"
        echo
        print_warning "Sign-in interrupted. CodeRabbit CLI is already installed."
        print_status "Run 'coderabbit auth login' when ready."
    }

    trap post_install_login_interrupted INT

    if [ -t 0 ]; then
        login_prompt_input="stdin"
    elif : 2>/dev/null </dev/tty; then
        login_prompt_input="tty"
    else
        echo
        print_auth "Sign in to CodeRabbit"
        print_auth "Run 'coderabbit auth login' when you're ready to connect your account."
        trap - INT
        return 0
    fi
    ...
    echo
    print_auth "Sign in to CodeRabbit"
    print_auth "Connect your account now so the CLI is ready for reviews."
    print_auth "You can skip this and run 'coderabbit auth login' later."
    printf "[AUTH] Start browser sign-in now? [Y/n] " >&2

    # Treat a failed read or EOF as "skip" rather than letting an empty
    # answer fall through to the default-yes branch.
    if [ "$login_prompt_input" = "tty" ]; then
        if ! IFS= read -r login_answer 2>/dev/null </dev/tty; then
            if [ "$POST_INSTALL_LOGIN_INTERRUPTED" = "1" ]; then
                trap - INT
                return 0
            fi
            echo
            print_status "Skipping sign-in. Run 'coderabbit auth login' when ready."
            trap - INT
            return 0
        fi
    else
        if ! IFS= read -r login_answer; then
            if [ "$POST_INSTALL_LOGIN_INTERRUPTED" = "1" ]; then
                trap - INT
                return 0
            fi
    ...
    normalized_login_answer=$(printf '%s' "$login_answer" | tr -d '[:space:]')

    case "$normalized_login_answer" in
        ""|y|Y|yes|YES|Yes)
            echo
            print_status "Opening CodeRabbit sign-in..."
            echo
            print_auth_separator
            # When the installer is run via `curl ... | sh`, fd 0 is the install
            # pipe. The login flow now fails fast on a non-readable stdin, so
            # rebind the child to /dev/tty whenever we read the prompt from it.
            if [ "$login_prompt_input" = "tty" ]; then
                if PATH="$BIN_DIR:$PATH" coderabbit auth login </dev/tty; then
                    POST_INSTALL_LOGIN_STATUS="success"
                elif [ "$POST_INSTALL_LOGIN_INTERRUPTED" = "1" ]; then
                    :
                else
                    POST_INSTALL_LOGIN_STATUS="failed"
                    print_warning "Login did not complete. Run 'coderabbit auth login' when ready."
                fi
            elif PATH="$BIN_DIR:$PATH" coderabbit auth login; then
                POST_INSTALL_LOGIN_STATUS="success"
            elif [ "$POST_INSTALL_LOGIN_INTERRUPTED" = "1" ]; then
                :
            else
